...
function visitIdent(_ref3) {
  ...
  if (identVal.__type === 'Null' || !val) {
    ...
    if (mixin) {
      ...
      var mixinValue = name === 'block' ? '@content;' : '@include ' + replaceFirstATSymbol(name, '') + ';';
      identLength--;
      return '\n' + before + mixinValue;
    }
  ...
  if (identVal.__type === 'Expression') {
    ...
    var _before = handleLinenoAndIndentation(identVal);
    ...
    identLength--;
    return '' + _before + replaceFirstATSymbol(name) + ': ' + trimFnSemicolon(expText) + '; ' + commentText;
  }
...
function visitEach(node) {
  ...
  exprNodes.forEach(function (node, idx) {
    var prefix = node.__type === 'Ident' ? COMPIL_CONFIT.scss.variable : '';
  ...
  before += blank;
  var block = visitBlock(node.block, blank).replace('' + COMPIL_CONFIT.scss.variable + node.key, '');
...
function converter(result) {
  ...
  var resultText = visitor(ast, options, globalVariableList, globalMixinList);
