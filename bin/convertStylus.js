...
function getFileReadCallback(input, output, options, callback) {
  return async function fileReadCallback(err, res) {
    if (err) throw err
    ...
    if (/\.styl$/.test(input)) {
      try {
        if (options.status === 'complete') {
          result = await converter(result, options, GLOBAL_VARIABLE_NAME_LIST, GLOBAL_MIXIN_NAME_LIST)
        } else {
...
function convertStylus(input, output, options, callback) {
  ...
  if (/\.styl$/.test(input) || /\.vue$/.test(input)) {
    fs.readFile(input, getFileReadCallback(input, output, options, callback))
  } else {
